...
2. **차트 데이터 리졸버** (`query getChartData(symbol: String!, startDate: String!, endDate: String!)`):
   - 클라이언트에서 요청한 특정 종목(`symbol`) 혹은 지수의 데이터를 ClickHouse에서 조회하세요.
   - 요청받은 **조회 기간(start_date ~ end_date)** 범위 내의 OHLCV 일봉 데이터와 보조 지표들을 클라이언트가 선택적으로 가져갈 수 있도록 GraphQL Object Type을 세밀하게 설계하세요.
